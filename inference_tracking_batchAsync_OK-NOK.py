...
MAXIMIZE_DISPLAY_SPEED = False # Speed up display when enough frames are queued
...
class Bottle:
    ...
    is_ok: bool = True  # True voor OK (label 0), False voor NOK (label 1)
    
    def __init__(self, x: float, y: float, yolo_id: int, is_ok: bool = True):
        self.x = x
        self.y = y
...
class Camera:
    ...
    def register_bottle(self, x, y, track_id, is_ok):
        if track_id in self.bottles:
            self.bottles[track_id].x = x
            self.bottles[track_id].y = y
            self.bottles[track_id].is_ok = is_ok  # Update OK/NOK status
            return False
        ...
        if track_id in self.temporary_bottles:
            ...
            self.temporary_bottles[track_id].is_ok = is_ok  # Update OK/NOK status
            if self.track_ids_seen[track_id] >= TEMPORAL_CUTOFF_THRESHOLD / self.get_allowed_frame_skip():
                ...
                status = "OK" if bottle.is_ok else "NOK"
                blabber(f"Bottle assigned index: {bottle.index} Track ID: {track_id} Camera: {self.name} Status: {status}")
            ...
            for result in results:
                ...
                if result.boxes is not None:
                    for camera in self.cameras:
                        ...
                        # Haal de class labels op (0 = OK, 1 = NOK)
                        ...
                        for box_index, box in enumerate(boxes):
                            ...
                            if x <= abs_x_center <= x + w and y <= abs_y_center <= y + h:
                                ...
                                # Bepaal of het een OK (0) of NOK (1) bottle is
                                is_ok = True
                                if class_ids is not None:
                                    is_ok = (class_ids[box_index] == 0)
                                
                                if camera.register_bottle(relative_x, relative_y, track_id, is_ok):
                                ...
                                if track_id in camera.temporary_bottles:
                                    ...
                                    bottle_id_color = (0, 0, 255)  # Blauw voor tijdelijke bottles
...
def main():
    cameras: list[Camera] = [
        Camera('Top', 'videos/14_55_top_cropped.mp4', start_skip=3),
        Camera('Front', 'videos/14_55_front_cropped.mp4', start_skip=0),

        Camera('Back Left', 'videos/14_55_back_left_cropped.mp4', start_skip=2),
        Camera('Back Right', 'videos/14_55_back_right_cropped.mp4', start_skip=1, start_index=-1),
    ]
